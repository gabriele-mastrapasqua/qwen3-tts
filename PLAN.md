...
- [x] PARITY-1 DONE for the matrix and the audited paths (aa3dad6): `docs/backend-matrix.md` is
      GENERATED by `tools/backend_matrix.py` from each build's own `--dispatch-map`, 48 features
      x 4 builds (M1, x86 portable/avx512vnni/amx) in the three states. Audited for silent
      backend forks: server batched path, prefork workers, scheduler/admission, Talker, CP,
      decoder, pool, regions, scratch lifetime, streaming, prefill, BLAS and thread ownership.
      Drift found and removed rather than documented: prefork affinity sliced logical CPUs
      (cb69996), BLAS ownership was advisory (576e487), the decoder conv panel was a constant
      (4bea77a). Remaining fork is the Arm persistent region, which is capability, not drift.
- [ ] PARITY-1b Common/shared code path parity, expressed as a
...
- [x] PARITY-3 DONE except hardware-blocked (aa3dad6 + 576e487): the ARM-oracle table in `docs/backend-matrix.md` gives every mature KleidiAI feature an explicit x86 status. Backend/dispatch functional parity, driven by ARM AS THE ORACLE. Do not ask the
...
## EPIC AMX — make the serving computation PRESENT AMX-suitable matrix work
Rewritten 2026-09-06 from the corrected census on canonical HEAD. Detail and evidence:
`.work/amx-native-epic.md`. Every task: QUESTION · EVIDENCE · IMPL · VALIDATION · NEXT.

THE FINDING THAT DEFINES THE EPIC: **the engine never enters AMX INT8 during real streaming
serving. Zero MACs.** The only AMX in production is BF16 tiles in the Talker prefill, 2.0% of
all MACs. 75% of MACs never reach the matmat dispatcher at all.

So the mission is NOT "tune the AMX gate" and NOT "see whether a dead path is 3% faster than
VNNI". It is: MAKE THE SERVING COMPUTATION PRESENT AMX-SUITABLE MATRIX WORK.

THE ARCHITECTURAL LEAD, from reading the mature ARM backend rather than theorising:
KleidiAI packs the RHS ONCE and ships TWO micro-kernels that consume THE SAME packed layout —
`qsi8d32p1x8_qsi4c32p4x8_1x4x32_neon_dotprod` for M=1 and
`qsi8d32p4x8_qsi4c32p4x8_16x4_neon_i8mm` for larger M (and `bf16p1x4_...1x36_neon_dot` vs
`bf16p8x4_...8x12_neon_mmla` for BF16). That is why `QWEN_MMK_KLEIDI_Q4` is the ONLY matrix
kernel in this engine with `min_b = 1`. On x86 the moment B drops to 1 we leave the matrix
world entirely for `matvec_int8`, a separate path that re-reads the weights in their original
form. **x86 abandons the packed representation at M=1; ARM does not.** That is the structural
gap, not a gate threshold.

INVALIDATED — DO NOT CITE: the earlier "AMX is 14% of MACs" and "Talker has ~85% AMX
...
- [ ] AMX-A Trusted current execution map. Q: what does the merged real server execute NOW?
      ...
      `qwen_mm` VNNI row (it did, and that contradiction should have halted the previous
      analysis); per-kernel shares must sum; counters must be counters, never masks; no stale
      current-row ownership. Emit `actual_amx_int8_mac_share`, `actual_amx_bf16_mac_share`,
      `actual_vnni_mac_share`, `actual_gemv_mac_share`, `outside_dispatcher_mac_share`.
      This is the denominator for everything below.
- [ ] AMX-B GEMV -> GEMM feasibility. FIRST-CLASS TASK, the heart of the epic.
      ...
      MANDATORY dependency classification before any aggregation is proposed —
      INDEPENDENT_NOW / SEQUENTIAL-AUTOREGRESSIVE / SHARED-WEIGHT-ONLY / ALGORITHM-CHANGING.
      Repeated work is not parallel work: do NOT fuse the CP's 16 steps merely because there
      are 16 of them; they are autoregressive.
...
- [ ] AMX-C Low-M AMX architecture — an engineering problem, not a permanent VNNI regime.
      ...
      memory traffic? oneDNN is the oracle: if it also loses badly at the same real shape the
...
- [ ] AMX-E BF16 as a first-class backend. We have ZERO historical AMX-BF16 evidence beyond the
      ...
      BF16 pays bf16 prep + AMX BF16 + fp32 out. Do not assume INT8 wins on peak TOPS.
- [ ] AMX-F W4 / future precision compatibility — feasibility and design only, no AutoRound.
      Audit where INT4/W4 weights are expanded or dequantised today and which AMX datatype
      would consume the resulting activation. The backend must not forbid W4A16 / W4A8 / W8A8 /
      mixed W4-W8.
- [ ] AMX-G Dataflow, packing and residency. Per hot AMX candidate: activation source · gather ·
      quant/convert · pack · weight representation · kernel · epilogue · scatter, each
      classified by lifetime (load-time · worker · request · frame · layer · projection).
      Previous prepack cost gigabytes and hurt TTFA and cache; the conclusion is NOT "prepack
      bad" but "find the MINIMAL representation and lifetime that lets AMX be fed".
- [ ] AMX-H Streaming-safe server policy. RTF is a primary metric again: TTFA 550 ms with
      STREAM_RTF 0.80 beats TTFA 250 ms with 1.20. Targets: zero underrun/starvation ·
      STREAM_RTF p95 < 1 with margin · p50 0.80-0.85 · TTFA <= 500-600 ms. Never spend
      steady-state budget to make first audio prettier. AMX work targets sustained concurrency.
- [ ] AMX-I 1.7B and 0.6B — same shape-family reasoning, shared implementation, shape-derived
      decisions, no model-name special cases.
- [ ] AMX-J Final server capacity, only after material implementation. Profiles AUTO/current ·
      VNNI-only · AMX-native, at C = 1,2,4,6. Report TTFA p50/p95 · STREAM_RTF p50/p95 · stream
      margin · throughput · AMX INT8 MAC share · AMX BF16 MAC share · VNNI/GEMV share ·
      outside-dispatcher share · effective B/M · worker balance · starvation. The question:
      can this Emerald Rapids host sustain materially more than the current C4 regime once the
      engine genuinely exploits AMX?

### What the corrected measurement invalidated

| old claim | current evidence | status |
|---|---|---|
| AMX is ~14% of MACs | per-kernel counters give 2.0% | INVALIDATED — read off an OR mask |
| INT8 AMX participates in production | 0 MACs, 0 calls | INVALIDATED |
| the B>=3 gate materially affected serving | it gates a path that never runs | IRRELEVANT under current serving |
| the rows/thread rule materially affected serving | same | IRRELEVANT under current serving |
| selective prepack acted on a meaningful fraction | it prepacks for a path that never runs | IRRELEVANT under current serving |
| Talker had ~85% AMX coverage | 11.1%, and all of it BF16 prefill | INVALIDATED |
| continuous batching lacked cross-request fusion | it already fuses; B 0.9-3.8 is the fused width | WRONG, and it stays wrong |
| the CP's 16 steps can be aggregated | autoregressive, not independent | WRONG — needs the dependency class first |
| the decoder is a side path | 76.6% of MACs, 0% AMX | INVALIDATED — it is the main event |
| AMX-2 should shrink because INT8 AMX is dead | the deadness IS the problem to solve | WRONG — the epic widens, not narrows |
...
- [ ] P0.6 [PARKED 2026-09-05 by explicit decision: not a priority, do NOT work on it] The golden references DO NOT hold
      ...
      the branch point that predates this work gives byte-identical WAVs on both trees, so
...
(the earlier research note that produced these questions is `.work/x86-dataflow-research.md`)
