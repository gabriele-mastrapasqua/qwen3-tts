...
| F16C | `--caps` now reports compile/runtime F16C; no qwen serving guard | Loader/utility inventory only | No production v2 call into Ingot F16/Q4_K paths | COMPILED BUT UNREACHABLE from v2 |
...
`third_party/ingot/src/kernels.c` includes AVX2/AVX512/NEON Q4_K, Q8, BF16, F16 and related CPU capability code (`third_party/ingot/src/cpu.c`). Ingot is primarily the vendored GGUF/SafeTensors loader and format utility; its optional math kernels are not a serving-backend contract. A source search of production qwen callers finds no calls to the Ingot `matvec/matmat/q4_k/q8_0/bf16_mat/f16_mat` entry points; only the dedicated Ingot benchmark/test code calls them. These remain `COMPILED BUT UNREACHABLE` from the v2 server, and must not be used as evidence of qwen AVX512-FP16/F16C support. The `--caps` report now exposes the host/build ISA bits separately so this distinction is visible without implying kernel quality.
...
| Capability report omitted common ISA bits | **FIXED — OBSERVABILITY ONLY** | `--caps` now prints compile/runtime F16C, BMI1, BMI2, AVX512VL, AVX512DQ, AVX512FP16/AMX-BF16 and Arm FP16/DOTPROD/I8MM/BF16 bits. These lines describe host/build availability; they do not claim a production caller or performance benefit. |
| Ingot SIMD was easy to mistake for a qwen serving backend | **CLARIFIED — LOADER/FORMAT LIBRARY** | Ingot remains the GGUF/SafeTensors and format utility dependency. Its Q4_K/Q8/BF16/F16 math entry points have no production qwen/v2 callers; the audit and `docs/ingot-migration-validation.md` now label them compiled-but-unreachable library code rather than serving kernels. |
