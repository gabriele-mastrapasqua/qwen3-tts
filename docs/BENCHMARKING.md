...
| first look at a NEW box for the streaming server, under a minute, no model | `make doctor` → `tools/doctor.py` (identity + roofs cached per hardware fingerprint + `make roof-matvec` int8 GEMV roof on the candidate worker mask + `--caps`/`--dispatch-map` gate + `--matmat-bench` shapes → PREDICTED `W x K` / batch cap / quantum / env set, a label on every number, `profile-draft.json` in `profiles/doctor/<run>/`) | before the first wave on any host; whenever the ISA, topology or build changes | **prediction only** — it tells the next wave where to start and never replaces `cpu-check` or a measurement |
