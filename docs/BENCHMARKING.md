...
| first look at a NEW box for the streaming server, under a minute, no model | `make doctor` → `tools/doctor.py` (on 32-core Arm Linux, first runs the simultaneous `roof_matvec_int8` 1×8/2×8/4×8 topology discriminator and archives `arm_gemv_scaling.json`; then identity + roofs cached per hardware fingerprint + `--caps`/`--dispatch-map` gate + `--matmat-bench` shapes → PREDICTED `W x K` / batch cap / quantum / env set, a label on every number, `profile-draft.json` in `profiles/doctor/<run>/`) | **first command before any serving wave or soak** on a new host; whenever the ISA, topology or build changes | **topology screen/prediction only** — its verdict is about the tested 4×8 shape, and it never replaces `cpu-check` or a serving measurement; see [Arm topology preflight](arm-topology-preflight.md) |
