...
| `QWEN_SD_RAG_FUSED_RESIDUAL_BLAS` | off | experimental ragged decoder epilogue path; keeps the 1x1 projection in the existing FP32 BLAS path but uses the residual copy as GEMM `C` with `beta=1`, removing the separate residual-add loop and c2 output allocation. It is independent from the AMX fused arm and leaves per-slot behavior unchanged |
