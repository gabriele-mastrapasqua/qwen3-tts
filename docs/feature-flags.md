...
| `QWEN_SD_FUSED_RESIDUAL` | off | experimental SQ-2d decoder path; for same-width 1x1 residual projections, adds the residual in the Design-D AMX epilogue while storing the projection result, avoiding the separate full-size residual-add pass. The output buffer remains separate so unsupported paths retain the existing fallback |
