...
| `QWEN_SD_DIRECT_CONVT` | off | experimental SQ-2 decoder preparation path; ragged transposed convolutions accumulate each existing per-tap GEMM directly into the useful output range and request-local carry instead of materializing/copying a full overlap buffer. It preserves the control path and is independently disable-able; allocation failure falls back to the existing implementation |
